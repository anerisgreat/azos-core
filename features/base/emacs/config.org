...
#+begin_src emacs-lisp
(defun azos/copy-file-name (&optional with-line)
  "Copy current buffer file path. With prefix arg, append :LINE."
  (interactive "P")
  (let ((fpath buffer-file-name))
    (if fpath
        (let ((result (if with-line
                          (format "%s:%d" fpath (line-number-at-pos))
                        fpath)))
          (kill-new result)
...
(define-key azos/global-minor-mode/open-keymap (kbd "f") #'azos/copy-file-name)
#+end_src
...
;; Run update func with timer
(defvar azos/tab-bar/update-timer
  (run-at-time t 5 'azos/tab-bar/update-func))
#+end_src
...
#+begin_src emacs-lisp
(defvar azos/modeline/indicators '()
  "Functions to call to produce extra mode-line indicator strings.
Each function takes no arguments and returns either a propertized
string, or nil if it has nothing to show for the current buffer.")

(defun azos/modeline/indicators-func ()
  "Concatenate all non-nil results of `azos/modeline/indicators'."
  (mapconcat 'identity
             (delq nil (mapcar 'funcall azos/modeline/indicators))
             ""))

(setq evil-normal-state-tag   (propertize " NORMAL  " 'face 'evil-hl-line-normal)
      evil-emacs-state-tag    (propertize " EMACS   " 'face 'evil-hl-line-emacs)
      evil-insert-state-tag   (propertize " INSERT  " 'face 'evil-hl-line-insert)
      evil-replace-state-tag  (propertize " REPLACE " 'face 'evil-hl-line-replace)
      evil-motion-state-tag   (propertize " MOTION  " 'face 'evil-hl-line-motion)
      evil-visual-state-tag   (propertize " VISUAL  " 'face 'evil-hl-line-visual)
      evil-operator-state-tag (propertize " OPERATE " 'face 'evil-hl-line-operator))
...
(use-package yasnippet
  :config
    ...
    (unless noninteractive (yas-global-mode 1))
)
#+end_src
...
Highlighting line with cursor, colored per Evil state via =evil-hl-line=.
...
#+begin_src emacs-lisp
(use-package counsel-tramp :defer t)
#+end_src
...
(defun azos/dired-open-externally ()
  "Open file at point with xdg-open. Copies remote files locally first."
  (interactive)
  (let* ((file (dired-get-file-for-visit))
         (local (or (file-local-copy file) file)))
    (start-process "xdg-open" nil "xdg-open" local)))

(with-eval-after-load 'dired
  (define-key dired-mode-map (kbd "C-c o") #'azos/dired-open-externally))
#+end_src
...
=peep-dired= provides quick image preview in a side window while browsing dired.
...
#+begin_src emacs-lisp
(with-eval-after-load 'org
  (add-to-list 'org-fold-show-context-detail '(isearch . tree))
  (add-to-list 'org-fold-show-context-detail '(default . tree)))
#+end_src
...
When a shell runs inside emacs (vterm, shell-mode, eshell), replace =$EDITOR=
with a mechanism that opens files directly in the current emacs session rather
than spawning a nested emacs process.
...
(add-to-list 'recentf-exclude (lambda (f) (file-directory-p (expand-file-name f))))
#+end_src
...
#+begin_src emacs-lisp
(defun azos/tabata/start (n_rounds)
  "Start tabata with 25/5 split. Read n_rounds if interactive."
  (interactive
   ;; Build a list of arguments for the function.
   (list (if current-prefix-arg
             (read-number "Number of rounds: ")   ; user supplied a prefix → ask
           1)))

  ;; Run command cabata start 25 5 n_rounds
    (let ((cmd (format "cabata start %s %s %s" (* 60 25) (* 60 5) n_rounds)))
      (start-process-shell-command "cabata" nil cmd))
  )

(defun azos/tabata/stop ()
  "Stop the tabata timer."
  (interactive)
  (start-process-shell-command "cabata" nil "cabata stop"))

(defun azos/tabata/status ()
  "Get tabata status"
  (interactive)
  (start-process-shell-command "cabata" nil "cabata status"))

(defun azos/tabata/quit ()
  "Quit tabata"
  (interactive)
  (start-process-shell-command "cabata" nil "cabata quit"))

;; Map functions to keymap azos/global-minor-mode/open-keymap
(define-key azos/global-minor-mode/open-keymap (kbd "t s") #'azos/tabata/start)
(define-key azos/global-minor-mode/open-keymap (kbd "t t") #'azos/tabata/stop)
(define-key azos/global-minor-mode/open-keymap (kbd "t S") #'azos/tabata/status)
(define-key azos/global-minor-mode/open-keymap (kbd "t q") #'azos/tabata/quit)
#+end_src
