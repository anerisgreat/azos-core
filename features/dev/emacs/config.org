...
;; eglot uses flymake; disable flycheck in Python buffers to avoid duplication
...
;; pet: detect correct Python executable per project (venv, poetry, pyenv, etc.)
(use-package pet
  :config
...
;; ruff format current buffer (stdin → stdout, replaces in place)
(defun azos/python/ruff-format ()
  "Format Python buffer with ruff."
  (interactive)
  (when (executable-find "ruff")
    (let ((saved-point (point)))
      (shell-command-on-region (point-min) (point-max) "ruff format -" t t nil t)
      (goto-char (min saved-point (point-max))))))

(with-eval-after-load 'python
  (define-key python-mode-map (kbd "C-c f") #'azos/python/ruff-format))
